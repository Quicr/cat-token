...
fn test_hmac_alg_id_is_5() {
    assert_eq!(
        ALG_HMAC256_256, 5,
        "HMAC 256/256 must use COSE alg ID 5 per RFC 9053"
    );
}
...
fn test_hmac_token_header_contains_alg_5() {
    ...
    // Parse the COSE_Mac0 structure to extract the protected header
    ...
    let arr = match value {
        ...
        _ => panic!("expected COSE_Mac0 tag 17"),
    };
    ...
    let header: ciborium::Value = ciborium::de::from_reader(header_cbor.as_slice()).unwrap();
    if let ciborium::Value::Map(map) = header {
        for (k, v) in &map {
